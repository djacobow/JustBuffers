...
if __name__ == '__main__':
    ...
    with open(sys.argv[3], 'r') as ifh:
        cpp_data = json.loads(ifh.read())

    with open(sys.argv[4], 'w') as ofh:
    ...
    for i in range(2):
        for j in range(2):
            ...
            cpp_t0 = cpp_data['t0s'][i][j]
            for field in ('fee', 'fi', 'fo', 'fum'):
                assert(cpp_t0[field] == t0[field])
 
            
    assert(cpp_data['blee'] == data['blee'])
